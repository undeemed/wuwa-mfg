...
def main():
    ...
    p.add_argument('--label-prefix',default='native-launch-order')
    p.add_argument('--command-probe',action='store_true',help='Also enable the command/owned-argument observer on the observed run.')
    a=p.parse_args();repo=Path(__file__).resolve().parents[2];demo=a.demo_dir.resolve();base=a.base.resolve()
    assert re.fullmatch('[a-z0-9-]{1,64}',a.label_prefix)
    ...
    for suffix,binary,digest in [('control',a.baseline_dll,a.baseline_sha256),('observed',a.observer_dll,a.observer_sha256)]:
        ...
        artifacts=['nr-model-capture','nr-buffer-probe.jsonl','nr-launch-contract.jsonl','nr-kernel-probe.csv','nr-command-probe.jsonl']
        assert not any((demo/n).exists() for n in artifacts)
        label=a.label_prefix+'-'+suffix;state=base/(label+'-state');state.mkdir(exist_ok=False)
        markers=['nr-model-capture.enable']+(['nr-kernel-probe.enable','nr-buffer-probe.enable'] if suffix=='observed' else [])
        if a.command_probe and suffix=='observed':markers.append('nr-command-probe.enable')
