...
def main():
    ...
    source=Path(__file__).with_name('chain_order_test.cu').read_bytes();directory=Path(torch.__file__).parent/'lib'
    with os.add_dll_directory(str(directory)):
        ...
        program=C.c_void_p();assert create(C.byref(program),source,b'chain_order_test.cu',0,None,None)==0
    ...
    report={'complete':True,'source_sha256':hashlib.sha256(source).hexdigest(),'cubin_sha256':hashlib.sha256(binary).hexdigest(),'cubin_bytes':len(binary),
        'options':[x.decode() for x in options],'gpu_execution':False,'original_workload_only':True}
