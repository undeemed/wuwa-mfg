...
1. Prepare the existing compatibility source and working NVIDIA NR installation
   using [BuildNeural.ps1 and the neural setup](neural-rendering.md). Keep its
...
2. Export a compatible **local** trained checkpoint. The exporter uses the existing
   ...
   ```powershell
   python research/neural-latency/export_dml_student.py `
...
3. Build the integration with Visual Studio 2022 C++ tools, Windows SDK, Git and
   ...
   ```powershell
   .\BuildStudentNeural.ps1 -CompatibilitySource C:\Private\nr-build\compat-source `
...
The [recorded test](../evidence/neural-model-research/student-hotswap-v1.json) used
...
- Both engines remain resident for switching. Up to three protected student
  ...
  NVIDIA fallback. WuWa gameplay validation and the separate final temporal test
  remain outstanding.
...
[original native backend](../research/neural-latency/native-student/),
[headless parity checker](../research/neural-latency/validate_dml_student.py), and
